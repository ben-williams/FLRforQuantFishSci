# FLR_latticeVsggplot.R - DESC
# FLR_latticeVsggplot.R
...
# Plotting FLR stocks and slots in Lattive and in ggplot using 
# data from Plaice stocks in N Atlantic ICES area IV
...
### chunk number 3: 
###################################################
# plot Catch data aggregated over all ages
print(xyplot(data~year, data=catch(ple4), type=c("g","l")))
print(ggplot(data=catch(ple4)) + geom_line(aes(year,data))+ylab("Catch"))


###################################################
### chunk number 4: 
###################################################
# plot Catch data by age group
print(xyplot(data~year, groups=age, data=catch.n(ple4), type=c("g","l")))
p<-ggplot(data=catch.n(ple4)) + geom_line(aes(year,data,col=factor(age)))+ylab("Catch")
print(p)
...
print(ggplot(data=catch.n(ple4)) + geom_point(aes(year,age,size=data)) )#+ scale_size_area())
